...
// Rasterbar effect consts.
                                        
.const RASTER_SHOW = $cfd
.const RASTER_MOVE_DIRECTION = $cfe
.const RASTER_START = $cff

// Clear chars effect consts.

.const ACTUAL_COLUMN_NO = $cfc
.const ACTUAL_ROW_NO = $cfb
...
raster_bar:
            lda rasterbar_colors, x             // Load value at label 'rasterbar_colors' plus x into a. if we don't add x, only the first
...
irq:
            ...
            cpx #$00
            beq clear_chars_state               // Change border color state.
            cpx #$01
            beq border_state
            cpx #$02
...
clear_chars_state:
            ldx RASTERBEAM_IRQ_COUNTER
            cpx #1                      // Clear char every 1/50 second (50 chars per second).
            beq clear_chars_clear_one

            jmp irq_main

clear_chars_clear_one:
            lda #$20                    // Spacebar char.
            ldx ACTUAL_COLUMN_NO
            sta $0420, x                // Clear next char from screen.
            sta $0500, x
            inc ACTUAL_COLUMN_NO        // Increment current column.
            
            jsr draw_ready_text                                     
            jsr reset_rasterbeam_counter
            
            lda ACTUAL_COLUMN_NO
            cmp #255
            beq next_state
